...
### 1. 🚀 Загрузчик (Bootstrap — Python/PyInstaller)
- **Папка:** `bootstrap/` (компилируется через `BUILD_BOOTSTRAP.bat` в `GanjaCraft.exe`).
- **Роль:** Скачивание, проверка цифровой подписи (Ed25519) и запуск основного Electron-клиента.
- **Умные обновления (Delta):**
  - Если лаунчер ставится с нуля (нет файла `.exe`), бутстрап скачивает полный архив (~100 МБ) с официального URL `fullUrl`.
  - Если лаунчер уже установлен, он скачивает только обновленные файлы ресурсов (`resources/app.asar`, ~1.35 МБ) с `url`. Это ускоряет обновление в 100 раз.

### 2. 💻 Клиент (Electron / JS)
...
### 3. ⚙️ Серверная часть (FastAPI-Бот — Python)
