...
/**
 * Получить текущие значения RAM из ползунков
 * @returns {{ min: string, max: string }}
 */
export function getRamSliderValues() {
    const sliderMin = document.getElementById('setting-ram-slider-min');
    const sliderMax = document.getElementById('setting-ram-slider-max');
    ...
    return {
        min: `${sliderMin.value}G`,
        max: `${sliderMax.value}G`
    };
}

/**
 * Установить значения RAM в ползунки
 * @param {string} memoryMin - Минимум (например "4G")
 * @param {string} memoryMax - Максимум (например "8G")
 */
export function setRamSliderValues(memoryMin, memoryMax) {
    const sliderMin = document.getElementById('setting-ram-slider-min');
    const sliderMax = document.getElementById('setting-ram-slider-max');
    ...
    // Триггерим обновление отображения
